...
<div class="drawer h-screen">
  ...
  <div class="z-30 flex flex-col justify-between h-100v w-16 bg-base-300 p-1 lg:absolute">
    <div>
      <a
        href="/forside"
    ...
    <div>
      ...
      <label
        tabindex="0"
        class="group btn mb-1 flex h-14 w-full flex-row flex-nowrap justify-between overflow-x-hidden p-0 pl-2 pr-1 transition-[width] duration-300 hover:w-44 whitespace-nowrap"
      >
        <Avatar id={"S" + cookie.userid} navn={$brugeren.navn} size="w-10" />
        <label class="btn btn-error btn-circle w-24 opacity-0 transition-all duration-300 group-hover:opacity-100" for="logud-modal">Log ud</label>
      </label>
      {/if}
    </div>
  </div>
