...
<div class="navbar bg-base-100 drop-shadow-xl mb-10">
  <div class="navbar-start">
    ...
    <p
      use:checkIfAuthed
      class="btn btn-ghost normal-case text-2xl mx-0 hidden sm:flex"
      on:keypress={() => {
        /*makes error go away*/
      }}
      on:click={() => {
  ...
  <div class="navbar-center flex sm:hidden">
    <p
      use:checkIfAuthed
      class="btn btn-ghost normal-case text-2xl mx-0 sm:hidden"
      on:keypress={() => {
        /*makes error go away*/
      }}
      on:click={() => {
...
<div
  use:removeLoader
  class="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 loader"
  id="loader"
>
  <button class="btn btn-circle btn-accent loading" />
</div>
