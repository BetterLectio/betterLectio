<script>
	...
	onMount(async() => {
		...
		if (await window.navigator.userAgent.includes('BetterLectio Mobile')) {
			...
			const _backendVersion = await fetch('https://raw.githubusercontent.com/BetterLectio/python-lectio/main/setup.py');
			// eslint-disable-next-line prefer-destructuring
			backendVersion = (await _backendVersion.text()).match(/VERSION = '\d+\.\d+\.\d+'/gu)[0].split("'")[1];
		}
	});
...
{#if appVersion && appVersion !== backendVersion}
