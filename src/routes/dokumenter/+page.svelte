<script>
	...
	get('/informationer').then(data => {
		$informationer = data;
		const _elever = {};
		for (const [navnOgKlasse, id] of Object.entries($informationer.elever)) {
			const { navn, klasse, userId } = parseStudentInfo(navnOgKlasse, id);

			_elever[`${navn} (${klasse})`] = userId;
		}
	...
	function calcExploreArr(data, id) {
			...
			if (item.id === id) {
				dataAfter.forEach(item2 => {
					...
					exploreArr.forEach(item3 => {
						if (item3.id === item2.id) exists = true;
					});
					if (exists) return;
		...
		exploreArr.forEach(item => {
			if (item.id === id) item.isArrowDown = true;
		});
	...
	async function clickHandler(element) {
		const { id } = element.srcElement.parentNode;

		// if ((await element.srcElement.parentNode.className.indexOf('breadcrumb')) > -1) {
		// 	breadcrumbs = [];
		// 	let done = false;
		// 	await computedBreadcrumbs.forEach(item => {
		// 		if (!done) {
		// 			breadcrumbs.push(item);
		// 			if (item.id === element.srcElement.parentNode.id) done = true;
		// 		}
		// 	});
		//
		...
		// 		if ((await element.srcElement.parentNode.className.indexOf('folder')) > -1) await breadcrumbs.push(lastClickedFolder);
		// 	}
		// } else {
	...
	function handleExploreClick(id, evt) {
		...
		<div class="h-full w-60 mr-2 hidden lg:inline">
			{#each exploreArr as item}
				<div
					...
					on:keyup={evt => handleExploreClick(item.id, evt)}
				>
