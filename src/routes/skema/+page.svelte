<script>
    ...
    viewDidMount: (view) => {
      ...
      btnNext.addEventListener("click", () => {
        if (getDesiredView() == "timeGridWeek") {
          ...
          let day = document.getElementsByClassName("ec-day")[0].innerHTML;
          console.log(day);
          if (day == "mandag") {
            weekOffset++;
            load();
  ...
  window.addEventListener("resize", function () {
    if (ec.getOption("view") != getDesiredView()) {
      loadedWeeks = [];
      weekOffset = 0;
      let events = ec.getEvents();

      events.forEach((modul) => {
        ec.removeEventById(modul.id);
      });
    }

    ec.setOption("view", getDesiredView());
  });
