<script>
	...
	<div class="flex flex-row justify-center items-center relative h-[912px] mt-96">
		<img src="/uno.png" alt="" class="max-h-[800px] absolute translate-x-14 -translate-y-14">
		<img src="/dos.png" alt="" class="max-h-[800px] absolute -translate-x-14 translate-y-14">
	</div>
					...
					<div class="flex flex-col lg:flex-row ">
						...
						<div class="bg-base-300 p-4 rounded-lg w-full">
							...
							<a
								...
								href="https://github.com/BetterLectio/Android/releases/download/{latestAndroidRelease.tag_name}/betterlectio.apk">Download</a>
						</div>
					</div>
				</div>
			{/if}
	...
	<div class="w-full hidden mt-56 md:flex items-center p-2 pl-4 h-10 bg-base-300">
		...
		<span>Lectio:</span>
		{#if lectioStatus}
			<span class={lectioStatus.status === "green" ? "ml-2 h-4 w-4 rounded-full bg-green-500" : lectioStatus.status === "yellow" ? "ml-2 h-4 w-4 rounded-full bg-yellow-500" : "ml-2 h-4 w-4 rounded-full bg-red-500"}></span>
		{:else}
			<span class="ml-2 h-4 w-4 rounded-full bg-yellow-500"></span>
		{/if}
		<span class="divider divider-horizontal"></span>
