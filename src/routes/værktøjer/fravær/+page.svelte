...
<h1 class="text-3xl font-bold mb-4">Fravær Regner</h1>
<div class="w-full max-w-xl rounded-lg bg-base-200 p-4 shadow-md sm:p-6 md:p-8">
  <label for="procent-range" class="mb-2 block font-medium"
    >Hvor meget procent fravær vil du ende med?</label
  >
  ...
  {#if possibleAmount - currentClasses.replace(",", ".") < 0}
    <p class="mt-2 text-xl lg:text-2xl">
      Du har allerede pjekket <span class="font-bold"
        >{Math.abs(possibleAmount - currentClasses.replace(",", "."))}</span
      > moduler for meget
    </p>
  {:else}
    <p class="mt-2 text-xl lg:text-2xl">
      Du kan pjekke <span class="font-bold">{possibleAmount - currentClasses.replace(",", ".")}</span> moduler
      mere
    </p>
  {/if}
  <br />
  <p class="mt-2 text-2xl">Stats</p>
  <p class="text-xl mt-2">Antal fraværende moduler: <span class="font-bold">{currentClasses}</span></p>
  <p class="text-xl mt-2">Totalt antal moduler: <span class="font-bold">{totalClasses}</span></p>
  <p class="text-xl mt-2">Fravær procent: <span class="font-bold">{currentFravær}%</span></p>
</div>
