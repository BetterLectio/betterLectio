<script>
  ...
  function findRoomIdAndRedirect(roomName) {
    ...
    for (const [key, value] of Object.entries(lokalerAndRoomId)) {
      if ("Lokale " + key == roomName) {
        goto(`/skema?id=${value.slice(0, 1) + value.slice(2)}`); //filter the value to not have "O" in the second index of the string then redirect to the skema page
      }
    }
  }
</script>
