<script>
	...
	async function loadKantine(kantineDomain) {
		const domain = kantineDomain.target
			? kantineDomain.target.value
			: kantineDomain;

		const _kantine = await fetch(`https://api.nemtakeaway.dk/api/v1/companies.php?cmd=get_data&domain=${domain}`, { headers: { accept: 'application/json, text/javascript, */*; q=0.01' } });
		let __kantine = (await _kantine.json()).company;
		[__kantine] = __kantine;

		const _varer = await fetch(`https://api.nemtakeaway.dk/api/v1/products.php?selfservice_token=${domain}&cmd=get_categories_list&get_products=true`,
			{ headers: { accept: 'application/json, text/javascript, */*; q=0.01' } });
		const varer = await _varer.json();
		__kantine.varer = varer.categories;

		return __kantine;
	}

	const kantine = loadKantine($indstillinger.kantineId);

	async function loadKantiner() {
		const _kantiner = await fetch('https://api.nemtakeaway.dk/api/v1/companies.php?cmd=get_company_group&group_slug=grabngo', { headers: { accept: 'application/json, text/javascript, */*; q=0.01' } });
...
{#if kantiner}
	...
	>
		...
		{#each kantiner.companies as kantineCompany}
			<option value={kantineCompany.domain}>
				{kantineCompany.name}
			</option>
		{/each}
	</select>
		...
		{#if kategori.products.length !== 0}
			...
			>
				<div
					...
					on:click={() => {
						closed[kategori.name] = !closed[kategori.name];
					}}
				>
