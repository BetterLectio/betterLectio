<script>
	...
	const SPØRGSMÅL_TYPER = {
		OVERSKRIFT: 'title',
		ET_SVAR: 'one-answer',
		FLERE_SVAR: 'multi-answer'
	};
	...
	const addOverskrift = () => {
		spørgesmålArr = [
			...spørgesmålArr,
			{
				type: SPØRGSMÅL_TYPER.OVERSKRIFT,
				overskrift: '',
				tekst: ''
			}
		];
	...
	const addFlereSvar = () => {
		spørgesmålArr = [
			...spørgesmålArr,
			{
				type: SPØRGSMÅL_TYPER.FLERE_SVAR,
				overskrift: '',
				tekst: '',
				svarMuligheder: ['', '', '', '']
			}
	...
	const removeSpørgsmål = i => {
		spørgesmålArr = spørgesmålArr.filter((_spørgsmål, index) => index !== i);
	};
	...
	const removeSvarmulighed = (i, j) => {
		spørgesmålArr[i].svarMuligheder = spørgesmålArr[i].svarMuligheder.filter((_mulighed, index) => index !== j);
	};
...
<div class="flex flex-row justify-between">
	<h1 class="heading">{titel === '' || titel === null ? 'Nyt spørgeskema' : titel}</h1>
	<div class="btn-group">
...
{#if !forhåndsvisning}
	<section class="h-fit w-full rounded-lg bg-base-200 p-4">
		<div class="form-control w-full">
			...
			<input type="text" placeholder="Nyt spørgeskema" bind:value={titel} id="titel" class="mb-6 input-bordered input w-full max-w-xs transition-all duration-300 {titel === '' || titel === null ? 'input-error' : ''}"/>
	...
	{#if spørgesmålArr.length !== 0}
		{#each spørgesmålArr as spørgsmål, i}
			<section class="mb-4 h-fit w-full rounded-lg bg-base-200 p-4" >
				<div class="flex flex-row justify-between">
					...
					<div class="btn-group">
						{#if spørgesmålArr.length > 1}
							{#if i !== 0}
								<div class=" btn-sm btn" on:click={() => {
									moveElement(i, -1);
								}} ><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-arrow-up" viewBox="0 0 16 16"><path fill-rule="evenodd" d="M8 15a.5.5 0 0 0 .5-.5V2.707l3.146 3.147a.5.5 0 0 0 .708-.708l-4-4a.5.5 0 0 0-.708 0l-4 4a.5.5 0 1 0 .708.708L7.5 2.707V14.5a.5.5 0 0 0 .5.5z"/></svg></div>
							{/if}
							{#if i !== spørgesmålArr.length - 1}
								<div class=" btn-sm btn" on:click={() => {
									moveElement(i, 1);
								}} ><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-arrow-down" viewBox="0 0 16 16"><path fill-rule="evenodd" d="M8 1a.5.5 0 0 1 .5.5v11.793l3.146-3.147a.5.5 0 0 1 .708.708l-4 4a.5.5 0 0 1-.708 0l-4-4a.5.5 0 0 1 .708-.708L7.5 13.293V1.5A.5.5 0 0 1 8 1z"/></svg></div>
							{/if}
						{/if}
						<div class="btn-error btn-sm btn" on:click={() => {
							removeSpørgsmål(i);
				...
				{#if spørgsmål.type === SPØRGSMÅL_TYPER.OVERSKRIFT || spørgsmål.type === 'Åbent svar'}
					<!-- ikke noget her-->
				{:else if spørgsmål.type === SPØRGSMÅL_TYPER.ET_SVAR || spørgsmål.type === SPØRGSMÅL_TYPER.FLERE_SVAR}
					<label class="label" for="svarmuligheder">
						<span class="label-text">Svarmuligheder:</span>
					</label>
					<div class="flex flex-col">
						{#each spørgsmål.svarMuligheder as svarMulighed, j}
							<label class="input-group">
								<input type="text" placeholder="Svarmulighed" id="svarmulighed" bind:value={svarMulighed} class="mb-2 input-bordered input input-sm w-full max-w-xs transition-all duration-300"/>
								<div class="btn btn-error btn-sm w-fit" on:click={() => {
									removeSvarmulighed(i, j);
								}}><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-trash3" viewBox="0 0 16 16"><path d="M6.5 1h3a.5.5 0 0 1 .5.5v1H6v-1a.5.5 0 0 1 .5-.5ZM11 2.5v-1A1.5 1.5 0 0 0 9.5 0h-3A1.5 1.5 0 0 0 5 1.5v1H2.506a.58.58 0 0 0-.01 0H1.5a.5.5 0 0 0 0 1h.538l.853 10.66A2 2 0 0 0 4.885 16h6.23a2 2 0 0 0 1.994-1.84l.853-10.66h.538a.5.5 0 0 0 0-1h-.995a.59.59 0 0 0-.01 0H11Zm1.958 1-.846 10.58a1 1 0 0 1-.997.92h-6.23a1 1 0 0 1-.997-.92L3.042 3.5h9.916Zm-7.487 1a.5.5 0 0 1 .528.47l.5 8.5a.5.5 0 0 1-.998.06L5 5.03a.5.5 0 0 1 .47-.53Zm5.058 0a.5.5 0 0 1 .47.53l-.5 8.5a.5.5 0 1 1-.998-.06l.5-8.5a.5.5 0 0 1 .528-.47ZM8 4.5a.5.5 0 0 1 .5.5v8.5a.5.5 0 0 1-1 0V5a.5.5 0 0 1 .5-.5Z"/></svg></div>
								{#if spørgesmålArr[i].svarMuligheder.length > 1}
									{#if j !== 0}
										<div class="btn-sm btn" on:click={() => {
											moveSvarmulighed(i, j, -1);
										}}><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-arrow-up" viewBox="0 0 16 16"><path fill-rule="evenodd" d="M8 15a.5.5 0 0 0 .5-.5V2.707l3.146 3.147a.5.5 0 0 0 .708-.708l-4-4a.5.5 0 0 0-.708 0l-4 4a.5.5 0 1 0 .708.708L7.5 2.707V14.5a.5.5 0 0 0 .5.5z"/></svg></div>
									{/if}
					...
					<div class="btn btn-sm" on:click={() => {
						addSvarmulighed(i);
					}}>Tilføj svarmulighed</div>
				{/if}
			</section>
		{/each}
	{:else}
...
{:else}
	...
	{#each spørgesmålArr as spørgesmål}
		<section class="mb-4 h-fit w-full rounded-lg bg-base-200 p-4">
			{#if spørgesmål.type === SPØRGSMÅL_TYPER.OVERSKRIFT}
				<h2 class="mb-2 text-2xl font-bold">{spørgesmål.overskrift}</h2>
				<p class="mb-2 whitespace-pre-line">{spørgesmål.tekst}</p>
			{:else if spørgesmål.type === SPØRGSMÅL_TYPER.ET_SVAR || spørgesmål.type === SPØRGSMÅL_TYPER.FLERE_SVAR}
				<h2 class="mb-2 text-2xl font-bold">{spørgesmål.overskrift}</h2>
				<p class="mb-2 whitespace-pre-line">{spørgesmål.tekst}</p>
				<div class="mt-2">
					<div class="form-control">
						{#each spørgesmål.svarMuligheder as svarMulighed}
							<label class="label cursor-pointer">
								<span class="label-text">{svarMulighed}</span>
								{#if spørgesmål.type === SPØRGSMÅL_TYPER.ET_SVAR}
									<input type="radio" name={spørgesmål} class="radio bg-neutral-content" disabled />
								{:else if spørgesmål.type === SPØRGSMÅL_TYPER.FLERE_SVAR}
