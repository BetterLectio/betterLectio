...
<div>
  ...
  <div class="flex flex-col sm:flex-row ">
    ...
    <input
      ...
      on:input={search}
    />
    <div class="dropdown dropdown-bottom dropdown-end hidden ml-4 lg:inline-flex">
      ...
      <ul tabindex="0" class="dropdown-content menu p-2 shadow bg-base-100 rounded-box w-52">
        <div class="form-control">
          <label class="label cursor-pointer">
            ...
            <input type="checkbox" checked="checked" class="checkbox" on:click={() => {opgaverIndstillinger.erFristRelativITabel ? opgaverIndstillinger.erFristRelativITabel = false : opgaverIndstillinger.erFristRelativITabel = true; localStorage.setItem("opgaver_indstillinger", JSON.stringify(opgaverIndstillinger));}} />
          </label>
        </div>
        <div class="form-control">
          <label class="label cursor-pointer">
            <span class="label-text">Vis hele beskrivelsen</span> 
            <input type="checkbox" checked="" class="checkbox" on:click={() => {opgaverIndstillinger.visHeleBeskrivelsenITabel ? opgaverIndstillinger.visHeleBeskrivelsenITabel = false : opgaverIndstillinger.visHeleBeskrivelsenITabel = true}} />
          </label>
        </div>
      </ul>
    </div>
        ...
        <tbody class="w-full">
          {#each _opgaver as opgave (opgave.exerciseid)}
            <tr class="" in:fade={{ duration: 200 }} out:fade={{ duration: 200 }}>
              ...
              <td class="text-left beskrivelse" id={opgave.exerciseid}>
                <div>
                  {opgave.opgavenote}
                </div>
              </td>
