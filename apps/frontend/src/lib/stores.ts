...
export const googleSyncStore = localStore<GoogleSyncSettings>(
	"googleSettings",
	{
		enabled: false,
...
export const homeworkStore = lectioDataStore<RawHomework[]>(
	"/lektier",
	"homework",
	[],
	(data) =>
...
export const roomsStore = lectioDataStore<Rooms>(
	"/lokale_dagsorden",
	"rooms",
	[],
	(data) => {
...
export const absenceStore = lectioDataStore<RawAbsence | null>(
	"/fravaer",
	"absence",
	null,
);

export const gradesStore = lectioDataStore<RawGrade | null>(
	"/karakterer",
	"grades",
	null,
);
...
>("/informationer", "information", null, (data) => {
	const students = Object.entries({ ...data.elever, ...data.lærere }).map(
		([name, id]) => {
			return {
				id,
	...
	const groups = Object.entries(data.hold_og_grupper ?? {}).map(
		([name]) => name,
	);
	return {
		students,
...
export const lessonsStore = localStore<{ [key: number]: RawLesson[] }>(
	"lessons",
	{},
);
