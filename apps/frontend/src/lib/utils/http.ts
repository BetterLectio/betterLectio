...
export async function uploadOpgave(formData: FormData, exerciseid: string) {
  console.log('uploading opgave');
  console.log(formData);
  console.log(exerciseid);
  const response = await fetch(`${LECTIO_API}/aflever_opgave?exerciseid=${exerciseid}`, {
    method: 'POST',
    body: formData,
    headers: { 'lectio-cookie': storeGet(authStore).cookie || '' }
  });
